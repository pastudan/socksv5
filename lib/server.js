...
var BUF_AUTH_NO_ACCEPT = new Buffer([0x05, 0xff]),
  BUF_REP_INTR_SUCCESS = new Buffer([
    0x05,
    REP.SUCCESS,
    0x00,
    0x01,
...
function Server(options, listener) {
  ...
  if (typeof options === "function") {
    ...
    options = undefined;
  } else if (typeof listener === "function") self.on("connection", listener);
  ...
  this._debug =
    options && typeof options.debug === "function" ? options.debug : undefined;
...
Server.prototype._onConnection = function (socket) {
  ...
  parser
    ...
    .on("request", function (reqInfo) {
      if (reqInfo.cmd !== "connect") return socket.end(BUF_REP_CMDUNSUPP);
      ...
      function accept(intercept) {
        ...
        if (socket.writable) {
          if (intercept) {
            ...
            return socket;
          } else proxySocket(socket, reqInfo, self.localAddress);
        }
      }
      function deny() {
        if (handled) return;
      ...
      proxySocket(socket, reqInfo, self.localAddress);
    });
...
function proxySocket(socket, req, localAddress) {
  dns.lookup(req.dstAddr, {family: 4}, function (err, dstIP) {
    if (err) {
    ...
    dstSock
      ...
      .on("connect", function () {
        ...
        if (socket.writable) {
          ...
          for (var i = 0; i < len; ++i, ++p) bufrep[p] = localbytes[i];
